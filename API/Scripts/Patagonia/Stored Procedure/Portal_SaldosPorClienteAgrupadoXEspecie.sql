if exists(select * from sysobjects where name = 'SaldosPorClienteAgrupadoXEspecie')
	drop procedure SaldosPorClienteAgrupadoXEspecie
go
    
CREATE procedure [dbo].[SaldosPorClienteAgrupadoXEspecie]     
...
begin    
	...
	create table #_tmp (    
	 ...
	 begin  
		...
		select     
			CASE 
				WHEN es.tesp_codigo not in ('A','F') THEN 'B'
				ELSE es.tesp_codigo
			END as 'Especie',
			CASE 
				WHEN es.tesp_codigo not in ('A','F') THEN 'Bonos'
				ELSE te.tesp_descrip
			END as 'Descripcion',
			0 as 'Porcentaje',
	...
	select
		...
		convert(int,(sum(SaldoValorizado) * 100 / (select sum(SaldoValorizado) From #_tmp))) as 'Porcentaje',
		sum(Saldo) as 'Saldo',
